...
async function addUser (user) {
  ...
  await Permission.query().insert({ user: _user.id })
...
async function updatePassword (id, password) {
  ...
  return User.query()
    .patch({ password: hash, password_reset_token: null })
...
async function generateCode (id) {
  ...
  while (true) {
    try {
      ...
      let result = await Code.query().insert(Code.fromJson(data))
...
async function toggleBandanna (id) {
  const bandanna = (await User.query().findById(id)).bandanna

  return User.query().patchAndFetchById(id, { bandanna: !bandanna })
}
...
async function getOZs () {
  ...
  const users = await Promise.all(
    ozs.map(async i => await User.query().where('id', i.user))
  )
  return users
}
