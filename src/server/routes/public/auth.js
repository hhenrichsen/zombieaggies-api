...
router.get('/auth/reset', async ctx => {
  ...
  return ctx.render('auth/reset', {
    error:
      token === undefined
        ? 'Invalid reset token please type it manually.'
        : undefined,
    token: token,
    csrf: ctx.csrf
  })
    ...
    .then(() =>
      passport.authenticate('local', (err, user) => {
        if (user) {
          ...
          return ctx.redirect(
            '/auth/register?error=A user with that email already exists%2E'
          )
        }
      })(ctx)
    )
  ...
  if (ctx.isAuthenticated()) {
    return ctx.redirect(
      `https://discordapp.com/api/oauth2/authorize?client_id=${
        ...
        `${ctx.request.origin}/auth/discord/callback`
...
router.get('/auth/status', async ctx => {
  if (ctx.isAuthenticated()) {
    if (
      ctx.req.user.tosAgree !== undefined &&
      ctx.req.user.tosAgree === false
    ) {
      return ctx.redirect('/start/tos')
    }
    if (
      ctx.req.user.active ||
      (ctx.query['ignore_redirect'] !== undefined &&
        ctx.query['ignore_redirect'] === 'true')
    ) {
...
router.get(
  '/auth/forgot-sent',
  async ctx => await ctx.render('auth/forgotSent.pug')
)
