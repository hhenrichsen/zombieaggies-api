...
router.get(`${BASE_URL}/buttons`, async ctx => {
  if (ctx.isAuthenticated()) {
    if (ctx.req.user && ctx.req.user.permissions.useAdminRoutes) {
...
router.get(`${BASE_URL}`, async ctx => {
  if (ctx.isAuthenticated()) {
    if (ctx.req.user && ctx.req.user.permissions.accessUserManagement) {
    ...
    if (ctx.req.user.permissions.useAdminRoutes) {
      ...
      if (id === 1) {
        ...
        let _res = await Promise.all(
          result.map(async item => (!(await tags.isOZ(item.id)) ? item : null))
        )
        result = _res.filter(i => i !== null)
      }
      if (result.length > 0) {
        result = result.map(i => i.username).join('; ')
      }
      ctx.status = 200
      ctx.body = result
      return Promise.resolve()
