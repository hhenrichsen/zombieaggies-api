...
var init = async function () {
    ...
    await fetch(`http://${hostname}/api/v1/teams`, { method: 'GET', async: false, headers: { "Content-Type": "application/json", } })
    ...
    await fetch(`http://${hostname}/api/v1/locations`, { method: 'GET', async: false, headers: { "Content-Type": "application/json", } })
        ...
        .then(json => {
            ...
            for (idx in data.locations) {
                if (!data.locations.hasOwnProperty(idx)) continue;
...
var getColor = (pos) => {
    return data.teams[pos.owner].color;
}
